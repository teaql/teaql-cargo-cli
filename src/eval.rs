...
pub fn evaluate(input: &Path, args: &EvalArgs, config: &ResolvedConfig) -> Result<i32> {
    ...
    let file_bytes = match fs::read(&upload_path) {
        Ok(b) => b,
        Err(e) => {
            eprintln!("error: failed to read upload file {}: {:#}", upload_path.display(), e);
    ...
    if let Some(ref out_path) = args.output {
        ...
        if let Err(e) = fs::write(out_path, &response_text) {
            eprintln!("warning: failed to write output file {}: {:#}", out_path.display(), e);
        }
    }
    ...
    if args.format.eq_ignore_ascii_case("json") {
        match serde_json::to_string_pretty(&eval_res) {
            Ok(json_str) => println!("{}", json_str),
            Err(_) => println!("{}", response_text),
        }
    } else {
        // Text format
        println!("KSML Evaluation");
        println!();
        println!("Solids: {}", eval_res.solids.len());
        println!("Warnings: {}", eval_res.warnings.len());
        println!("Errors: {}", eval_res.errors.len());

        if !eval_res.warnings.is_empty() {
            println!();
            println!("Warnings");
            for item in &eval_res.warnings {
                println!("- [{}] {} at {}", item.rule_id, item.title, item.path);
